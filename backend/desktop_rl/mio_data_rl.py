...
class Mio_API_get_data(Thread):
    def __init__(self):
        ...
        self.last_msg_l = json.dumps({'x': 0, 'y': 0, 's': 0})
        self.last_msg_r = json.dumps({'x': 0, 'y': 0, 's': 0})
        self.is_open = False
        ...
        self.msg = '0'
        self.sleep_time = 0
        self.decode_message_l = {'x': 0, 'y': 0, 's': 0}  # new decode fun
        self.decode_message_r = {'x': 0, 'y': 0, 's': 0}
    ...
    def string_to_json(self, line):
        ...
        if i_list[0] == 73:
            ...
            if i_list[0] == 48:
                self.decode_message_l['y'] = i_list[2]
                self.decode_message_l['x'] = i_list[3]
            elif i_list[0] == 144:
                self.decode_message_l['s'] = i_list[4]
            elif i_list[0] == 80:
                print(f'Заряд:{i_list[2]}%')
        elif i_list[1] == 1:
            if i_list[0] == 48:
                self.decode_message_r['y'] = i_list[2]
                self.decode_message_r['x'] = i_list[3]
            elif i_list[0] == 144:
                self.decode_message_r['s'] = i_list[4]
            elif i_list[0] == 80:
                print(f'Заряд:{i_list[2]}%')

        return self.decode_message_l, self.decode_message_r
            ...
            try:
                ...
                while True:
                    ...
                    try:
                        ...
                        # print(f'Messege:{msg}')
                        self.last_msg_l = msg_l
                        self.last_msg_r = msg_r
